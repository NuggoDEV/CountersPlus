...
namespace CountersPlus.Config
{
    internal class ConfigDefaults
    {
        internal static Dictionary<string, ConfigModel> Defaults = new Dictionary<string, ConfigModel>()
        {
            ...
            { "Personal Best", new PBConfigModel() { Enabled = true, Position = ICounterPositions.BelowMultiplier, Index = 1, DecimalPrecision = 2, TextSize = 2, UnderScore = true, HideFirstScore = false } },
            { "Notes Left", new NotesLeftConfigModel() { Enabled = false, Position = ICounterPositions.AboveHighway, Index = -1, LabelAboveCount = false, } },
            { "Fail", new FailConfigModel() { Enabled = false, Position = ICounterPositions.AboveCombo, Index = 0, ShowRestartsInstead = false, } },
        };
